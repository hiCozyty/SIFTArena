...
import type { useOpencodeChat } from "@/hooks/use-opencode-chat"

type PlaybookChatTabProps = ReturnType<typeof useOpencodeChat>

export function PlaybookChatTab({ messages, isGenerating, isToolExecuting, error, sendMessage, stopGenerating, submitQuestionAnswer }: PlaybookChatTabProps) {
  ...
  const handleAnswerQuestion = useCallback((answers: Record<number, string>) => {
    ...
    if (!lastAssistantMsg?.id) {
      console.error("[playbook-chat-tab] handleAnswerQuestion: no assistant message found")
      return
    }
        ...
        <div className="h-full overflow-y-auto px-4 pt-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" ref={containerRef} onScroll={handleScroll}>
          {isEmpty ? (
            <div className="flex h-full flex-col items-center justify-center gap-3">
              <Button onClick={() => {
                sendMessage("Create a noise template")
              }}>
                Create a noise template
              </Button>
            </div>
          ) : (
            <MessageList
