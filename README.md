...
**Root cause**: Persistent artifact contamination. A comsvcs.dll dump from the *previous playbook run* remained on disk, and the model incorrectly attributed it to the current (Dumpert) session. This highlights the need for VM snapshot resets between runs and stricter temporal filtering within the attack window.
