...
async function testFullPipeline() {
  console.log("\n=== Full Pipeline: Leverage Procdump for lsass memory ===")

  const abilityId = "0ef4cc7b-611c-4237-b20b-db36b6906554"
  const abilityName = "Leverage Procdump for lsass memory"
  const group = `test-${Date.now()}`
  const taskName = `CalderaSandcat-${group}`
  ...
  try {
    ...
    console.log(`\n  [5/5] Verifying dump file (procdump writes lsass.exe_*.dmp to working dir)...`)
    let dumpExists = false
    try {
      const dumpCheck = await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "$file = Get-ChildItem \'C:\\Windows\\system32\\lsass.exe_*.dmp\' -ErrorAction SilentlyContinue | Select-Object -First 1; if ($file) { Write-Host \\"EXISTS $([math]::Round($file.Length/1024/1024,1))MB name=$($file.Name)\\" } else { Write-Host \\"MISSING\\" }"')
      console.log(`    => C:\\Windows\\system32\\lsass.exe_*.dmp: ${dumpCheck.trim()}`)
    ...
    try {
      await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "Remove-Item \'C:\\Windows\\system32\\lsass.exe_*.dmp\' -Force -ErrorAction SilentlyContinue; Remove-Item \'C:\\Users\\Public\\Procdump.zip\' -Force -ErrorAction SilentlyContinue; Remove-Item \'C:\\Users\\Public\\temp\' -Recurse -Force -ErrorAction SilentlyContinue; Write-Host \'CLEANED\'"')
      console.log("    dump files and procdump artifacts deleted")
    } catch {}
...
async function main() {
  console.log("testAbility — Leverage Procdump for lsass memory\n")
