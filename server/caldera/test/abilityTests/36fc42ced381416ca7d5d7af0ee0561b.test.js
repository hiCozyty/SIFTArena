...
async function testFullPipeline() {
  console.log("\n=== Full Pipeline: Dump LSASS.exe using imported Microsoft DLLs ===")

  const abilityId = "36fc42ced381416ca7d5d7af0ee0561b"
  const abilityName = "Dump LSASS.exe using imported Microsoft DLLs"
  const group = `test-${Date.now()}`
  const taskName = `CalderaSandcat-${group}`

  // xordump.exe is downloaded by the ability command itself via Invoke-WebRequest — no external payload or prereq step needed
  // Command uses PowerShell (Test-Path, Invoke-WebRequest, xordump.exe invocation) — must execute via PowerShell executor
  ...
  try {
    ...
    // Clean up xordump.exe from previous runs
    try {
      await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "Remove-Item \'C:\\Windows\\Temp\\xordump.exe\' -Force -ErrorAction SilentlyContinue; Write-Host \'XORDUMP_CLEANED\'"')
    } catch {}
    console.log("    => clean")
    ...
    // No command modification needed — xordump is self-downloading, no PathToAtomicsFolder references
    ...
    try {
      const dumpCheck = await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "if (Test-Path \'C:\\Windows\\Temp\\lsass-xordump.t1003.001.dmp\') { $f=Get-Item \'C:\\Windows\\Temp\\lsass-xordump.t1003.001.dmp\'; Write-Host \\"EXISTS $([math]::Round($f.Length/1024/1024,1))MB\\" } else { Write-Host \\"MISSING\\" }"')
      dumpExists = dumpCheck.includes("EXISTS")
      dumpSize = dumpCheck.trim()
      console.log(`    => Dump file: ${dumpSize}`)
    } catch (err) {
    ...
    // xordump XORs the dump then deletes the intermediate file per its design.
    // Success: dump file exists on disk OR link completed successfully (status=0).
    const passed = dumpExists || linkStatus === 0
    const reason = dumpExists ? `dump=${dumpSize}` : linkStatus === 0 ? "link completed (xordump deleted intermediate dump)" : "no dump + link failed"
    ...
    console.log("\n  [cleanup] Removing agent, task, process, dump, and xordump.exe...")
    ...
    try {
      await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "Remove-Item \'C:\\Windows\\Temp\\lsass-xordump.t1003.001.dmp\' -Force -ErrorAction SilentlyContinue; Write-Host \'DUMP_DELETED\'"')
      console.log("    dump file deleted")
    } catch {}
    try {
      await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "Remove-Item \'C:\\Windows\\Temp\\xordump.exe\' -Force -ErrorAction SilentlyContinue; Write-Host \'XORDUMP_DELETED\'"')
      console.log("    xordump.exe deleted")
    } catch {}
