...
async function testFullPipeline() {
  ...
  const abilityId = "60bb6f8468aa98b75be2521861a164d5"
  const abilityName = "Dump LSASS.exe Memory using Out-Minidump.ps1"
  const group = `test-${Date.now()}`
  const taskName = `CalderaSandcat-${group}`
...
async function main() {
  console.log("testAbility — Out-Minidump.ps1 LSASS Dumper Test\n")
