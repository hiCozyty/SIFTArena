...
async function testFullPipeline() {
  console.log("\n=== Full Pipeline: Dump LSASS.exe Memory using NanoDump ===")

  const abilityId = "990a4629d154622098b70ba50384051f"
  const abilityName = "Dump LSASS.exe Memory using NanoDump"
  const group = `test-${Date.now()}`
  const taskName = `CalderaSandcat-${group}`
  ...
  try {
    ...
    // Path is unquoted: PathToAtomicsFolder\..\ExternalPayloads\nanodump.x64.exe -w "%temp%\nanodump.dmp"
    ...
    console.log(`\n  [6/6] Verifying dump file in C:\\Windows\\Temp\\nanodump.dmp...`)
    let dumpExists = false
    let dumpSize = ""
    try {
      const dumpCheck = await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "if (Test-Path \'C:\\Windows\\Temp\\nanodump.dmp\') { $f=Get-Item \'C:\\Windows\\Temp\\nanodump.dmp\'; Write-Host \\"EXISTS $([math]::Round($f.Length/1024/1024,1))MB\\" } else { Write-Host \\"MISSING\\" }"')
    ...
    console.log("\n  [cleanup] Removing agent, task, process, dump, and binary...")
    ...
    try {
      await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "Remove-Item \'C:\\Windows\\Temp\\nanodump.dmp\' -Force -ErrorAction SilentlyContinue; Write-Host \'DUMP_DELETED\'"')
...
// ─── Ability Specifics ──────────────────────────────────────────────────────
//
//   - nanodump.x64.exe uses syscalls and invalid dump signature to avoid detection
//   - Direct dump via -w flag: writes lsass memory to a .dmp file
//   - Dump output: %temp%\nanodump.dmp → C:\Windows\Temp\nanodump.dmp (SYSTEM's temp)
//   - win_prereq: downloads nanodump.x64.exe via WinRM
//   - Command PathToAtomicsFolder replaced with C:\Users\Public\nanodump.x64.exe (unquoted path)
//   - No EULA acceptance needed (unlike ProcDump)
//   - Agent runs as SYSTEM (Scheduled Task) for SeDebugPrivilege to access LSASS
//   - Cleanup removes binary and dump file

// ─── Main ────────────────────────────────────────────────────────────────────

async function main() {
  console.log("testAbility — Dump LSASS.exe Memory using NanoDump\n")
  ...
  console.log(`\n  NanoDump ability specifics:`)
  console.log(`    - nanodump.x64.exe uses syscalls and invalid dump signature to avoid detection`)
  console.log(`    - Dump file: C:\\Windows\\Temp\\nanodump.dmp (SYSTEM's %temp% expansion)`)
    console.log(`    - nanodump deployed to C:\\Users\\Public\\ on Windows via win_prereq (WinRM)`)
  console.log(`    - PathToAtomicsFolder replaced with local binary path (unquoted)`)
