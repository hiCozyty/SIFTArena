...
async function testFullPipeline() {
  console.log("\n=== Full Pipeline: Create Mini Dump of LSASS.exe using ProcDump ===")

  const abilityId = "54643bf2018c940ca5fc21097884d2d4"
  const abilityName = "Create Mini Dump of LSASS.exe using ProcDump"
  const group = `test-${Date.now()}`
  const taskName = `CalderaSandcat-${group}`
  ...
  try {
    ...
    console.log(`\n  [4c/6] Deploying procdump.exe to Windows...`)
    const destPath = `C:\\Users\\Public\\procdump.exe`
    ...
    // ── Step 6: Verify dump file at C:\Windows\Temp\lsass_dump.dmp ──
    console.log(`\n  [6/6] Verifying dump file in C:\\Windows\\Temp\\lsass_dump.dmp...`)
    try {
...
async function main() {
  console.log("testAbility — Create Mini Dump of LSASS.exe using ProcDump\n")
